      ...
      <template #item="{element}">
        ...
        <v-col cols="12" :lg="element.showMore ? 3 : 2"  @click="element.showMore = !element.showMore; save()" :id="'char-' + element.name+'-'+element.realm"><!-- /v-col>v-for="value in chars" -->
          <v-card class="pa-2" :class="{ char: element.cheese}">
            <v-card-title class="char__name">
              <v-btn 
                :href.prevent="`/gear/${element.realm}/${element.name}`"
                :target="`${element.realm}-${element.name}`"
                @click.stop
                variant="text"
                class="ml-0 pl-0"
                >
                {{ element.name }}
                <v-icon @click.stop="dismissVault(element)" class="text-subtitle-1 ml-1" icon="mdi-treasure-chest-outline" v-if="element?.runs?.mythic_plus_previous_weekly_highest_level_runs?.length > 0 && element?.vaultDismissedForPeriod != euCurrentPeriod.period"></v-icon>
