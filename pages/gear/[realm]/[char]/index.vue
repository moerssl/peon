...
const routeChar = route.params.char
const routeRealm = route.params.realm
const charName = ref(routeChar || "Echskalibur")
const realm = ref(routeRealm || "Malorne")
const lang = ref("de")
...
const loadBis = async (playedClass, spec) => {
  try {
      if (playedClass == undefined || spec == undefined)
        return 
...
const reloadEquipmentJson = async() => {
  ...
  router.push(`/gear/${realm.value}/${charName.value}`)
  try {
...
onMounted(() => {
  if (routeChar == undefined || routeRealm == undefined) return
  reloadEquipmentJson()
})
/*
